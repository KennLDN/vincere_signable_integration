...
const logger = require('../../utils/logger')('ENVELOPE CANCELLED WEBHOOK');
const ReferenceLogs = require('../../models/ReferenceLogs');

const router = express.Router();

router.post('/ec-webhook', bodyParser.text({ type: '*/*' }), async (req, res) => {
  try {
    ...
    for (let condition of updateConditions) {
      ...
      const updatedDoc = await ReferenceLogs.findOneAndUpdate(condition, {
        $set: {
          [keyToUpdateStatus]: -1,
          [keyToUpdateMsg]: 'envelope was cancelled by user'
        }
      }, { new: true });
