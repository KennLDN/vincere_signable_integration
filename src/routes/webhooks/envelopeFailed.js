...
const logger = require('../../utils/logger')('ENVELOPE FAILED WEBHOOK');
const ReferenceLogs = require('../../models/ReferenceLogs');

const router = express.Router();

router.post('/ef-webhook', bodyParser.text({ type: '*/*' }), async (req, res) => {
  try {
    ...
    for (let condition of updateConditions) {
      ...
      const updatedDoc = await ReferenceLogs.findOneAndUpdate(condition, {
        $set: {
          [keyToUpdateStatus]: -1,
          [keyToUpdateMsg]: 'envelope failed to send'
        }
      }, { new: true });
