...
const logger = require('../../utils/logger')('ENVELOPE SIGNED WEBHOOK');
const ReferenceLogs = require('../../models/ReferenceLogs');

const router = express.Router();

router.post('/es-webhook', bodyParser.text({ type: '*/*' }), async (req, res) => {
  try {
    ...
    for (let condition of updateConditions) {
      ...
      const updatedDoc = await ReferenceLogs.findOneAndUpdate(condition, {
        $set: {
          [keyToUpdateStatus]: 2,
          [keyToUpdateMsg]: 'envelope has been signed'
        }
      }, { new: true });
