...
const logger = require('../../utils/logger')('ENVELOPE OPENED WEBHOOK');
const ReferenceLogs = require('../../models/ReferenceLogs');

const router = express.Router();

router.post('/eo-webhook', bodyParser.text({ type: '*/*' }), async (req, res) => {
  try {
    ...
    for (let condition of updateConditions) {
      ...
      const updatedDoc = await ReferenceLogs.findOneAndUpdate(condition, {
        $set: {
          [keyToUpdateStatus]: 2,
          [keyToUpdateMsg]: 'envelope has been opened'
        }
      }, { new: true });
